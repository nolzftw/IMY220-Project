...
async function main() {
  try {
    ...
    app.patch('/api/users/:email', async (req, res) => {
      const email = req.params.email;
      const { name, gender, bio, password, link } = req.body;
    ...
    app.get('/api/users/:email/friends', async (req, res) => {
      ...
      try {
        ...
        res.status(200).json(friends.map((friend) => friend._id)); // Return the emails
      } catch (err) {
    ...
    app.get('/api/users/:email1/mutual/:email2', async (req, res) => {
      const email1 = req.params.email1;
      const email2 = req.params.email2;

      try {
        const user1 = await usersCollection.findOne({ _id: email1 });
        const user2 = await usersCollection.findOne({ _id: email2 });

        if (!user1 || !user2) {
          return res.status(404).json({ message: 'One or both users not found' });
        }

        const isMutualFriend = user1.friends.includes(email2) && user2.friends.includes(email1);
        res.status(200).json({ mutualFriends: isMutualFriend });
      } catch (err) {
        res.status(500).json({ message: 'Server error, please try again later' });
      }
    });
